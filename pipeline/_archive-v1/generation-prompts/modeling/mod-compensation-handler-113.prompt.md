# Authoring task: mod-compensation-handler-113

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-compensation-handler-113.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-compensation-handler-113` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `medium`
- **style**: `recall`
- **camundaVersion**: `"8.8"`
...
- **Heading focus** (optional steer): "Interrupting compensation handlers"
...
**Section heading**: Interrupting compensation handlers
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-compensation-handler-113.json** and nothing else.
2. The id MUST be `mod-compensation-handler-113`.
