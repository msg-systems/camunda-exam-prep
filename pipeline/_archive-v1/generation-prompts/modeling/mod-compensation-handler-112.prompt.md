# Authoring task: mod-compensation-handler-112

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-compensation-handler-112.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-compensation-handler-112` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `hard`
- **style**: `concept`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/modeler/bpmn/compensation-handler/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Multi-instance activity as compensation handler"
...
**File**: `pipeline/sources/modeling/compensation-handler.md`
**Public URL**: https://docs.camunda.io/docs/components/modeler/bpmn/compensation-handler/
**Section heading**: Multi-instance activity as compensation handler
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-compensation-handler-112.json** and nothing else.
2. The id MUST be `mod-compensation-handler-112`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/modeler/bpmn/compensation-handler/`.
