# Authoring task: mod-embedded-subprocesses-33

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-embedded-subprocesses-33.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-embedded-subprocesses-33` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `hard`
- **style**: `concept`
- **camundaVersion**: `"8.8"`
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-embedded-subprocesses-33.json** and nothing else.
2. The id MUST be `mod-embedded-subprocesses-33`.
