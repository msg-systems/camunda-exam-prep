# Authoring task: mod-script-tasks-24

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-script-tasks-24.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-script-tasks-24` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `medium`
- **style**: `scenario`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/modeler/bpmn/script-tasks/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Defining a script task {#defining-a-task}"
...
**File**: `pipeline/sources/modeling/script-tasks.md`
**Public URL**: https://docs.camunda.io/docs/components/modeler/bpmn/script-tasks/
**Section heading**: Defining a script task {#defining-a-task}
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-script-tasks-24.json** and nothing else.
2. The id MUST be `mod-script-tasks-24`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/modeler/bpmn/script-tasks/`.
...
Overwrite `pipeline/generation/outputs/modeling/mod-script-tasks-24.json` with a corrected JSON document and stop.
