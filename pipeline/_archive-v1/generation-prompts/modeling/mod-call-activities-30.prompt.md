# Authoring task: mod-call-activities-30

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-call-activities-30.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-call-activities-30` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `easy`
- **style**: `concept`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/modeler/bpmn/call-activities/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Defining the called process"
...
**File**: `pipeline/sources/modeling/call-activities.md`
**Public URL**: https://docs.camunda.io/docs/components/modeler/bpmn/call-activities/
**Section heading**: Defining the called process
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-call-activities-30.json** and nothing else.
2. The id MUST be `mod-call-activities-30`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/modeler/bpmn/call-activities/`.
...
Overwrite `pipeline/generation/outputs/modeling/mod-call-activities-30.json` with a corrected JSON document and stop.
