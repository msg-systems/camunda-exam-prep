# Authoring task: mod-escalation-events-101

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-escalation-events-101.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-escalation-events-101` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `hard`
- **style**: `scenario`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/modeler/bpmn/escalation-events/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Catching the escalation"
...
**File**: `pipeline/sources/modeling/escalation-events.md`
**Public URL**: https://docs.camunda.io/docs/components/modeler/bpmn/escalation-events/
**Section heading**: Catching the escalation
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-escalation-events-101.json** and nothing else.
2. The id MUST be `mod-escalation-events-101`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/modeler/bpmn/escalation-events/`.
