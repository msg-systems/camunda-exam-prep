# Authoring task: mod-error-events-93

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-error-events-93.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-error-events-93` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `medium`
- **style**: `scenario`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/modeler/bpmn/error-events/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Business error vs. technical error"
...
**File**: `pipeline/sources/modeling/error-events.md`
**Public URL**: https://docs.camunda.io/docs/components/modeler/bpmn/error-events/
**Section heading**: Business error vs. technical error
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-error-events-93.json** and nothing else.
2. The id MUST be `mod-error-events-93`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/modeler/bpmn/error-events/`.
...
Overwrite `pipeline/generation/outputs/modeling/mod-error-events-93.json` with a corrected JSON document and stop.
