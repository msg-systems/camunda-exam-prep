# Authoring task: mod-terminate-events-104

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/modeling/mod-terminate-events-104.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mod-terminate-events-104` (use exactly this id)
- **topic**: `modeling`
- **difficulty**: `medium`
- **style**: `scenario`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/modeler/bpmn/terminate-events/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Inside a subprocess"
...
**File**: `pipeline/sources/modeling/terminate-events.md`
**Public URL**: https://docs.camunda.io/docs/components/modeler/bpmn/terminate-events/
**Section heading**: Inside a subprocess
...
1. Write the JSON to **pipeline/generation/outputs/modeling/mod-terminate-events-104.json** and nothing else.
2. The id MUST be `mod-terminate-events-104`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/modeler/bpmn/terminate-events/`.
