# Authoring task: cfg-process-instance-migration-49

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-process-instance-migration-49.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-process-instance-migration-49` (use exactly this id)
...
- **Heading focus** (optional steer): "Migrate job worker user tasks to Camunda user tasks"
...
**Section heading**: Migrate job worker user tasks to Camunda user tasks
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-process-instance-migration-49.json** and nothing else.
2. The id MUST be `cfg-process-instance-migration-49`.
