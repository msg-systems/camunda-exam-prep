# Authoring task: cfg-feel-built-in-functions-string-150

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-string-150.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-feel-built-in-functions-string-150` (use exactly this id)
...
- **Heading focus** (optional steer): "extract(string, pattern)"
...
**Section heading**: extract(string, pattern)
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-string-150.json** and nothing else.
2. The id MUST be `cfg-feel-built-in-functions-string-150`.
