# Authoring task: cfg-expressions-11

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-expressions-11.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-expressions-11` (use exactly this id)
- **topic**: `configuring-processes`
- **difficulty**: `medium`
- **style**: `scenario`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/concepts/expressions/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Expressions vs. static values"
...
**File**: `pipeline/sources/configuring-processes/expressions.md`
**Public URL**: https://docs.camunda.io/docs/components/concepts/expressions/
**Section heading**: Expressions vs. static values
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-expressions-11.json** and nothing else.
2. The id MUST be `cfg-expressions-11`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/concepts/expressions/`.
