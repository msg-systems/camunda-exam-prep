# Authoring task: cfg-feel-built-in-functions-string-148

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-string-148.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-feel-built-in-functions-string-148` (use exactly this id)
...
- **Heading focus** (optional steer): "replace(input, pattern, replacement, flags)"
...
**Section heading**: replace(input, pattern, replacement, flags)
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-string-148.json** and nothing else.
2. The id MUST be `cfg-feel-built-in-functions-string-148`.
