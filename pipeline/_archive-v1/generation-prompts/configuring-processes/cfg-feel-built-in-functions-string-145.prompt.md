# Authoring task: cfg-feel-built-in-functions-string-145

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-string-145.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-feel-built-in-functions-string-145` (use exactly this id)
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-string-145.json** and nothing else.
2. The id MUST be `cfg-feel-built-in-functions-string-145`.
