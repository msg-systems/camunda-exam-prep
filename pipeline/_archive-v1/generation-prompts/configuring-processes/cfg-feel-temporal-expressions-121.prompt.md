# Authoring task: cfg-feel-temporal-expressions-121

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-feel-temporal-expressions-121.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-feel-temporal-expressions-121` (use exactly this id)
...
- **Heading focus** (optional steer): "Literal"
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-feel-temporal-expressions-121.json** and nothing else.
2. The id MUST be `cfg-feel-temporal-expressions-121`.
