# Authoring task: cfg-feel-built-in-functions-context-167

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-context-167.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-feel-built-in-functions-context-167` (use exactly this id)
...
- **Heading focus** (optional steer): "context merge(contexts)"
...
**Section heading**: context merge(contexts)
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-context-167.json** and nothing else.
2. The id MUST be `cfg-feel-built-in-functions-context-167`.
