# Authoring task: cfg-feel-built-in-functions-list-152

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-list-152.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `cfg-feel-built-in-functions-list-152` (use exactly this id)
...
- **Heading focus** (optional steer): "any(list)"
...
1. Write the JSON to **pipeline/generation/outputs/configuring-processes/cfg-feel-built-in-functions-list-152.json** and nothing else.
2. The id MUST be `cfg-feel-built-in-functions-list-152`.
