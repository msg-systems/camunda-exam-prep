# Authoring task: dec-decision-table-input-25

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/decisions-business-rules/dec-decision-table-input-25.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `dec-decision-table-input-25` (use exactly this id)
...
- **Heading focus** (optional steer): "Input expression"
...
**Section heading**: Input expression
...
1. Write the JSON to **pipeline/generation/outputs/decisions-business-rules/dec-decision-table-input-25.json** and nothing else.
2. The id MUST be `dec-decision-table-input-25`.
...
Overwrite `pipeline/generation/outputs/decisions-business-rules/dec-decision-table-input-25.json` with a corrected JSON document and stop.
