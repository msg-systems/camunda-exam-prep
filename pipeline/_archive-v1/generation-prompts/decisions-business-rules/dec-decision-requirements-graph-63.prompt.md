# Authoring task: dec-decision-requirements-graph-63

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-63.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `dec-decision-requirements-graph-63` (use exactly this id)
...
1. Write the JSON to **pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-63.json** and nothing else.
2. The id MUST be `dec-decision-requirements-graph-63`.
...
Overwrite `pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-63.json` with a corrected JSON document and stop.
