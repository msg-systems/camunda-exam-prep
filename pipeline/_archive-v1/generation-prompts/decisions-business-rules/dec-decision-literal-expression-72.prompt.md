# Authoring task: dec-decision-literal-expression-72

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/decisions-business-rules/dec-decision-literal-expression-72.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `dec-decision-literal-expression-72` (use exactly this id)
...
- **Heading focus** (optional steer): "Decision id"
...
1. Write the JSON to **pipeline/generation/outputs/decisions-business-rules/dec-decision-literal-expression-72.json** and nothing else.
2. The id MUST be `dec-decision-literal-expression-72`.
...
Overwrite `pipeline/generation/outputs/decisions-business-rules/dec-decision-literal-expression-72.json` with a corrected JSON document and stop.
