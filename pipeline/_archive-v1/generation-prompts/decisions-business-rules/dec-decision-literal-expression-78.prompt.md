# Authoring task: dec-decision-literal-expression-78

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/decisions-business-rules/dec-decision-literal-expression-78.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `dec-decision-literal-expression-78` (use exactly this id)
...
- **Heading focus** (optional steer): "Variable"
...
1. Write the JSON to **pipeline/generation/outputs/decisions-business-rules/dec-decision-literal-expression-78.json** and nothing else.
2. The id MUST be `dec-decision-literal-expression-78`.
...
Overwrite `pipeline/generation/outputs/decisions-business-rules/dec-decision-literal-expression-78.json` with a corrected JSON document and stop.
