# Authoring task: dec-decision-requirements-graph-69

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-69.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `dec-decision-requirements-graph-69` (use exactly this id)
...
- **Heading focus** (optional steer): "Knowledge source"
...
**Section heading**: Knowledge source
...
1. Write the JSON to **pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-69.json** and nothing else.
2. The id MUST be `dec-decision-requirements-graph-69`.
...
Overwrite `pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-69.json` with a corrected JSON document and stop.
