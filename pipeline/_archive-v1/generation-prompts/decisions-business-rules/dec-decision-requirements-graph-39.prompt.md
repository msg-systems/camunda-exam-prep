# Authoring task: dec-decision-requirements-graph-39

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-39.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `dec-decision-requirements-graph-39` (use exactly this id)
...
- **Heading focus** (optional steer): "Decision requirements graph name"
...
**Section heading**: Decision requirements graph name
...
1. Write the JSON to **pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-39.json** and nothing else.
2. The id MUST be `dec-decision-requirements-graph-39`.
