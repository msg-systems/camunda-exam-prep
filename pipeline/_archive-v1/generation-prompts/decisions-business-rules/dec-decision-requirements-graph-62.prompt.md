# Authoring task: dec-decision-requirements-graph-62

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-62.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `dec-decision-requirements-graph-62` (use exactly this id)
- **topic**: `decisions-business-rules`
- **difficulty**: `medium`
- **style**: `concept`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/modeler/dmn/decision-requirements-graph/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Decision requirements graph id"
...
**File**: `pipeline/sources/decisions-business-rules/decision-requirements-graph.md`
**Public URL**: https://docs.camunda.io/docs/components/modeler/dmn/decision-requirements-graph/
**Section heading**: Decision requirements graph id
...
1. Write the JSON to **pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-62.json** and nothing else.
2. The id MUST be `dec-decision-requirements-graph-62`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/modeler/dmn/decision-requirements-graph/`.
...
Overwrite `pipeline/generation/outputs/decisions-business-rules/dec-decision-requirements-graph-62.json` with a corrected JSON document and stop.
