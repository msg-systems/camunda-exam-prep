# Authoring task: frm-forms-element-library-number-26

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/forms/frm-forms-element-library-number-26.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `frm-forms-element-library-number-26` (use exactly this id)
- **topic**: `forms`
- **difficulty**: `hard`
- **style**: `scenario`
- **camundaVersion**: `"8.8"`
...
1. Write the JSON to **pipeline/generation/outputs/forms/frm-forms-element-library-number-26.json** and nothing else.
2. The id MUST be `frm-forms-element-library-number-26`.
