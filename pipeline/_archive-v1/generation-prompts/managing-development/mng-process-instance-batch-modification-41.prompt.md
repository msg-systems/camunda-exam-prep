# Authoring task: mng-process-instance-batch-modification-41

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/managing-development/mng-process-instance-batch-modification-41.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mng-process-instance-batch-modification-41` (use exactly this id)
...
- **Heading focus** (optional steer): "Move selected process instances from one flow node to another"
...
**Section heading**: Move selected process instances from one flow node to another
...
1. Write the JSON to **pipeline/generation/outputs/managing-development/mng-process-instance-batch-modification-41.json** and nothing else.
2. The id MUST be `mng-process-instance-batch-modification-41`.
