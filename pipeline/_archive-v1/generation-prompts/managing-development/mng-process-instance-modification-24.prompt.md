# Authoring task: mng-process-instance-modification-24

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/managing-development/mng-process-instance-modification-24.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mng-process-instance-modification-24` (use exactly this id)
...
- **Heading focus** (optional steer): "Move all running tokens from one flow node to another"
...
**Section heading**: Move all running tokens from one flow node to another
...
1. Write the JSON to **pipeline/generation/outputs/managing-development/mng-process-instance-modification-24.json** and nothing else.
2. The id MUST be `mng-process-instance-modification-24`.
