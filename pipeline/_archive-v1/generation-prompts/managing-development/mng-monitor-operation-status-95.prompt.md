# Authoring task: mng-monitor-operation-status-95

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/managing-development/mng-monitor-operation-status-95.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `mng-monitor-operation-status-95` (use exactly this id)
...
1. Write the JSON to **pipeline/generation/outputs/managing-development/mng-monitor-operation-status-95.json** and nothing else.
2. The id MUST be `mng-monitor-operation-status-95`.
