# Authoring task: ext-testing-process-definitions-176

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-testing-process-definitions-176.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-testing-process-definitions-176` (use exactly this id)
...
- **Heading focus** (optional steer): "Drive the process and assert the state"
...
**Section heading**: Drive the process and assert the state
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-testing-process-definitions-176.json** and nothing else.
2. The id MUST be `ext-testing-process-definitions-176`.
