# Authoring task: ext-service-integration-patterns-160

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-service-integration-patterns-160.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-service-integration-patterns-160` (use exactly this id)
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-service-integration-patterns-160.json** and nothing else.
2. The id MUST be `ext-service-integration-patterns-160`.
