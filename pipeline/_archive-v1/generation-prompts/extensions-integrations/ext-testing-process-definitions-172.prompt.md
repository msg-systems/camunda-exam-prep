# Authoring task: ext-testing-process-definitions-172

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-testing-process-definitions-172.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-testing-process-definitions-172` (use exactly this id)
...
- **Heading focus** (optional steer): "Testing scopes"
...
**Section heading**: Testing scopes
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-testing-process-definitions-172.json** and nothing else.
2. The id MUST be `ext-testing-process-definitions-172`.
