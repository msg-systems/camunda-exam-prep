# Authoring task: ext-routing-events-to-processes-188

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-routing-events-to-processes-188.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-routing-events-to-processes-188` (use exactly this id)
...
- **Heading focus** (optional steer): "Sending messages to other processes"
...
**Section heading**: Sending messages to other processes
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-routing-events-to-processes-188.json** and nothing else.
2. The id MUST be `ext-routing-events-to-processes-188`.
