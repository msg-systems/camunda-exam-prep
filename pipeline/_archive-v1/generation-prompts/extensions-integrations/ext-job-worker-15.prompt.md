# Authoring task: ext-job-worker-15

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-job-worker-15.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-job-worker-15` (use exactly this id)
- **topic**: `extensions-integrations`
- **difficulty**: `medium`
- **style**: `concept`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/apis-tools/java-client/job-worker/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "How job workers work"
...
**File**: `pipeline/sources/extensions-integrations/job-worker.md`
**Public URL**: https://docs.camunda.io/docs/apis-tools/java-client/job-worker/
**Section heading**: How job workers work
...
- Performing long-polling job activation when activating jobs larger than the [maximum message size](../../self-managed/components/orchestration-cluster/zeebe/configuration/gateway.md#zeebegatewaynetwork).
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-job-worker-15.json** and nothing else.
2. The id MUST be `ext-job-worker-15`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/apis-tools/java-client/job-worker/`.
