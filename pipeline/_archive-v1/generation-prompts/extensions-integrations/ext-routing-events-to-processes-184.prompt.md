# Authoring task: ext-routing-events-to-processes-184

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-routing-events-to-processes-184.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-routing-events-to-processes-184` (use exactly this id)
- **topic**: `extensions-integrations`
- **difficulty**: `hard`
- **style**: `recall`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/best-practices/development/routing-events-to-processes/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Reacting to process-internal events"
...
**File**: `pipeline/sources/extensions-integrations/routing-events-to-processes.md`
**Public URL**: https://docs.camunda.io/docs/components/best-practices/development/routing-events-to-processes/
**Section heading**: Reacting to process-internal events
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-routing-events-to-processes-184.json** and nothing else.
2. The id MUST be `ext-routing-events-to-processes-184`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/best-practices/development/routing-events-to-processes/`.
...
- [quality-option-rationale-leak] option text contains its own verdict/rationale @ options[0]
- [quality-option-rationale-leak] option text contains its own verdict/rationale @ options[1]
- [quality-option-rationale-leak] option text contains its own verdict/rationale @ options[2]
- [quality-option-rationale-leak] option text contains its own verdict/rationale @ options[3]

Overwrite `pipeline/generation/outputs/extensions-integrations/ext-routing-events-to-processes-184.json` with a corrected JSON document and stop.
