# Authoring task: ext-testing-process-definitions-173

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-testing-process-definitions-173.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-testing-process-definitions-173` (use exactly this id)
...
- **Heading focus** (optional steer): "Technical setup using Spring"
...
**Section heading**: Technical setup using Spring
...
2. Use the [Camunda Spring Boot Starter](../../../apis-tools/camunda-spring-boot-starter/getting-started.md).
3. Use `@CamundaSpringProcessTest` to start a process engine.
4. Ensure you have Docker installed locally to use [TestContainers](../../../apis-tools/testing/getting-started.md#prerequisites), which is the easiest way to run tests.
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-testing-process-definitions-173.json** and nothing else.
2. The id MUST be `ext-testing-process-definitions-173`.
