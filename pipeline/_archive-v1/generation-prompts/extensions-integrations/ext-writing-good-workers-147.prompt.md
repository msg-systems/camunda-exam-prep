# Authoring task: ext-writing-good-workers-147

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-writing-good-workers-147.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-writing-good-workers-147` (use exactly this id)
...
- **Heading focus** (optional steer): "Java"
...
The [Camunda Spring Boot Starter](/apis-tools/camunda-spring-boot-starter/getting-started.md) provides a more elegant way of writing this, but also uses a normal worker from the Java client underneath. In this case, your code might look like this:
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-writing-good-workers-147.json** and nothing else.
2. The id MUST be `ext-writing-good-workers-147`.
