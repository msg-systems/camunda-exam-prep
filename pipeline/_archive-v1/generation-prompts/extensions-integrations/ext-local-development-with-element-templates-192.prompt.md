# Authoring task: ext-local-development-with-element-templates-192

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-local-development-with-element-templates-192.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-local-development-with-element-templates-192` (use exactly this id)
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-local-development-with-element-templates-192.json** and nothing else.
2. The id MUST be `ext-local-development-with-element-templates-192`.
