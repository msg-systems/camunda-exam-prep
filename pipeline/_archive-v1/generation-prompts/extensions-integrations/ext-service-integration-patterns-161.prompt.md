# Authoring task: ext-service-integration-patterns-161

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-service-integration-patterns-161.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-service-integration-patterns-161` (use exactly this id)
...
- **Heading focus** (optional steer): "Message type on the wire != BPMN message type"
...
**Section heading**: Message type on the wire != BPMN message type
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-service-integration-patterns-161.json** and nothing else.
2. The id MUST be `ext-service-integration-patterns-161`.
