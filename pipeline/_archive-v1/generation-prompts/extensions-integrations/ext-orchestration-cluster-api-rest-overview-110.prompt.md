# Authoring task: ext-orchestration-cluster-api-rest-overview-110

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-orchestration-cluster-api-rest-overview-110.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-orchestration-cluster-api-rest-overview-110` (use exactly this id)
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-orchestration-cluster-api-rest-overview-110.json** and nothing else.
2. The id MUST be `ext-orchestration-cluster-api-rest-overview-110`.
