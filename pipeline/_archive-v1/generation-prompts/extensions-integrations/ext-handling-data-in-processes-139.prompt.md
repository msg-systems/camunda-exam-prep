# Authoring task: ext-handling-data-in-processes-139

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/extensions-integrations/ext-handling-data-in-processes-139.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `ext-handling-data-in-processes-139` (use exactly this id)
...
- **Heading focus** (optional steer): "Storing just the relevant data"
...
**Section heading**: Storing just the relevant data
...
1. Write the JSON to **pipeline/generation/outputs/extensions-integrations/ext-handling-data-in-processes-139.json** and nothing else.
2. The id MUST be `ext-handling-data-in-processes-139`.
