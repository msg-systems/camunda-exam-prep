# Authoring task: con-inbound-10

You are writing one Camunda 8.8 Certified Developer exam question. Output **only** the JSON file at:

    pipeline/generation/outputs/connectors/con-inbound-10.json

No other files. No commentary. Valid JSON, no trailing commas, no comments.

## Work item

- **id**: `con-inbound-10` (use exactly this id)
- **topic**: `connectors`
- **difficulty**: `easy`
- **style**: `recall`
- **camundaVersion**: `"8.8"`
- **docs[0].url**: `https://docs.camunda.io/docs/components/connectors/use-connectors/inbound/`
- **docs[0].title**: short page title (look at the source's H1 below)
- **Heading focus** (optional steer): "Creating the connector event"
...
**File**: `pipeline/sources/connectors/inbound.md`
**Public URL**: https://docs.camunda.io/docs/components/connectors/use-connectors/inbound/
**Section heading**: Creating the connector event
...
1. Write the JSON to **pipeline/generation/outputs/connectors/con-inbound-10.json** and nothing else.
2. The id MUST be `con-inbound-10`.
3. The docs[0].url MUST be `https://docs.camunda.io/docs/components/connectors/use-connectors/inbound/`.
...
Overwrite `pipeline/generation/outputs/connectors/con-inbound-10.json` with a corrected JSON document and stop.
